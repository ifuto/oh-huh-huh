...
export function createIsland() {
  ...
  function chalet(x: number, z: number, w: number, d: number, h: number, roofHeight: number, roofMaterial = 'roof', customY?: number) {
    ...
    for (const side of [-1, 1]) {
      for (let a = -w / 2 + .25; a <= w / 2; a += Math.max(1.4, w / 5)) {
      ...
      for (let a = -d / 2 + .3; a <= d / 2; a += 1.7) box(x + side * (w / 2 + .02), y + h / 2 + .7, z + a, .17, h, .17, 'timber');
    }
    ...
    for (let dz = -depth; dz <= depth + .1; dz += .72) for (const side of [-1, 1]) beam(v(x, eave + roofHeight + .07, z + dz), v(x + side * half, eave + .07, z + dz), .07, 'timber');
    for (const side of [-1, 1]) {
    ...
    for (let a = -w/2; a <= w/2+.1; a += 1.1) box(x+a, y+1.4, z+d/2+2.5, .1, 1.3, .1, 'timber');
  ...
  for(let i=0;i<20;i++) put(new THREE.IcosahedronGeometry(.5+rand()*.6,0),'foam','water',v(-17+(rand()-.5)*5,-21-rand()*5,44+(rand()-.5)*3),new THREE.Euler(),v(1,.5,1));
  for(let i=0;i<30;i++) { const a=rand()*Math.PI*2; const x=-2+Math.cos(a)*12.3, z=12+Math.sin(a)*9.3; put(new THREE.IcosahedronGeometry(.35+rand()*.65,0),'stone','paths',v(x,Math.max(lakeY,groundHeight(x,z))+.15,z),new THREE.Euler(),v(1.3,.6,1)); }
  ...
  path([[-9,-5],[-4,0],[10,1],[18,-1],[23,-3]],0.95);
  path([[-4,0],[-15,3],[-22,10],[-23,20],[-17,26],[-9,27],[0,25],[12,25],[24,29]],0.85);
  path([[10,1],[15,8],[15,16],[12,25]],0.85);
  path([[-22,10],[-31,13],[-33,4],[-31,-10],[-26,-20]],0.7);
  path([[23,-3],[31,5],[31,17],[24,29]],0.7);
  ...
  let trees=0;
  for(let i=0;i<2600&&trees<240;i++){const x=(rand()-.5)*94,z=(rand()-.5)*94,a=Math.atan2(z,x);if(Math.hypot(x,z)>radius(a)-2||noTree(x,z))continue;if(Math.hypot(x,z)<24&&rand()<.65)continue;tree(x,z,.65+rand()*.8,rand()>.43);trees++;}
  // Rocks, meadow flowers, shrubs and hanging vines at the exposed cliff.
  for(let i=0;i<115;i++){const a=rand()*Math.PI*2,rr=radius(a)*(.7+rand()*.27),x=Math.cos(a)*rr,z=Math.sin(a)*rr;if(noTree(x,z))continue;put(new THREE.IcosahedronGeometry(.5+rand()*.8,0),i%3===0?'stone':'leaf3','vegetation',v(x,groundHeight(x,z)+.3,z),new THREE.Euler(rand(),rand(),rand()),v(1.2,.65,1));}
  for(let i=0;i<180;i++){const x=(rand()-.5)*85,z=(rand()-.5)*85;if(Math.hypot(x,z)>43||noTree(x,z))continue;put(new THREE.IcosahedronGeometry(.1+rand()*.13,0),i%4===0?'pink':'flower','vegetation',v(x,groundHeight(x,z)+.22,z));}
  for(let i=0;i<58;i++){const a=rand()*Math.PI*2,r=radius(a),x=Math.cos(a)*r,z=Math.sin(a)*r;const length=2+rand()*10;for(let j=0;j<length;j+=1.1)put(new THREE.IcosahedronGeometry(.5+rand()*.6,0),i%2?'leaf1':'leaf2','vegetation',v(x*(1-j*.006),groundHeight(x,z)-j,z*(1-j*.006)),new THREE.Euler(),v(1,1.35,.7));}
